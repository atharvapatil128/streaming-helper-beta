...
- Replace the passive helper button with the supplied heart recommendation
  asset while a title is detected.
...
- Preserve the existing recommendation inbox and comfort-pick helper when no
  streamable title is detected.
...
2. On a likely title page, the heart icon appears with a title-specific tooltip.
