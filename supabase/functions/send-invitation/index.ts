...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  if (
    ...
    return jsonResponse({ code: 'SERVER_MISCONFIGURED', message: 'Server misconfigured' }, 500);
  }
  ...
  if (!authHeader?.startsWith('Bearer ')) {
    return jsonResponse({ code: 'UNAUTHENTICATED', message: 'Missing Authorization header' }, 401);
  }
  const jwt = authHeader.slice('Bearer '.length).trim();
  if (!jwt) {
    return jsonResponse({ code: 'UNAUTHENTICATED', message: 'Empty bearer token' }, 401);
  }
  ...
  if (userError || !userData?.user) {
    return jsonResponse({ code: 'UNAUTHENTICATED', message: 'Invalid or expired session' }, 401);
  }
  ...
  if (typeof rawEmail !== 'string') {
    return jsonResponse({ code: 'INVALID_EMAIL', message: 'An email address is required.' }, 400);
  }
  const email = rawEmail.trim().toLowerCase();
  if (email.length === 0 || email.length > MAX_EMAIL_LENGTH || !isValidEmail(email)) {
    return jsonResponse({ code: 'INVALID_EMAIL', message: 'Enter a valid email address.' }, 400);
  }
  if (inviterEmail && email === inviterEmail) {
    return jsonResponse({ code: 'CANNOT_INVITE_SELF', message: "You can't invite yourself." }, 400);
  }
  ...
  try {
    ...
    console.error('send-invitation: profile lookup failed', (err as { message?: string })?.message);
  }
  ...
  try {
    ...
    if (existingProfile) {
      return jsonResponse(
        { code: 'ACCOUNT_EXISTS', message: 'This person already has a Streaming Helper account.' },
        409,
      );
    }
  } catch (err) {
    console.error('send-invitation: account-exists check failed', (err as { message?: string })?.message);
    return jsonResponse({ code: 'LOOKUP_FAILED', message: 'Could not process the invitation.' }, 500);
  }
  ...
  try {
    ...
    if (pending) {
      ...
      if (!expired) {
        ...
        return jsonResponse({ status: 'already_pending', expiresAt: pending.expires_at }, 200);
      }
    ...
    console.error('send-invitation: pending lookup/revoke failed', (err as { message?: string })?.message);
    return jsonResponse({ code: 'LOOKUP_FAILED', message: 'Could not process the invitation.' }, 500);
  }
  ...
  try {
    ...
    if ((count ?? 0) >= RATE_LIMIT_MAX) {
      return jsonResponse(
        { code: 'RATE_LIMITED', message: 'You have sent too many invitations. Please try again later.' },
        429,
      );
    }
  } catch (err) {
    console.error('send-invitation: rate-limit count failed', (err as { message?: string })?.message);
    return jsonResponse({ code: 'LOOKUP_FAILED', message: 'Could not process the invitation.' }, 500);
  }
  ...
  try {
    const { data: inserted, error: insErr } = await admin
      .from('invitations')
      .insert({ inviter_id: inviterId, invitee_email: email, token_hash: tokenHash, status: 'pending' })
      .select('id, expires_at')
      .single();
    ...
    if (insErr) {
      ...
      if ((insErr as { code?: string }).code === '23505') {
        const { data: race } = await admin
          ...
          .maybeSingle();
        if (race) {
          return jsonResponse({ status: 'already_pending', expiresAt: race.expires_at }, 200);
        }
      }
      throw insErr;
    }
    ...
    console.error('send-invitation: insert failed', (err as { message?: string })?.message);
    return jsonResponse({ code: 'INSERT_FAILED', message: 'Could not create the invitation.' }, 500);
  }
  ...
  try {
    ...
    emailOk = false;
    console.error('send-invitation: Resend request threw', (err as { message?: string })?.message);
  }

  // ── 12. Email failure cleanup ─────────────────────────────────────────────
  if (!emailOk) {
    console.error('send-invitation: email send failed', { providerStatus, providerCode });
    await admin
      .from('invitations')
      .update({ status: 'revoked', responded_at: new Date().toISOString() })
      .eq('id', invitationId)
      .eq('status', 'pending');
    return jsonResponse(
      { code: 'EMAIL_SEND_FAILED', message: 'We could not send the invitation email. Please try again.' },
      502,
    );
  }
