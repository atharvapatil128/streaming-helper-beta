import {
  ...
  sha256Hex,
} from "../extension-login/core.ts";
import {
  calculateUsernameCredentialDelayMs,
  classifyAuthFailure,
  resolveUsernameForAuth,
} from "../extension-login/service.ts";
import { allowedAppOrigin, parseAppOrigin } from "./core.ts";

const REQUEST_TIMEOUT_MS = 5_000;
const configuredOrigin = parseAppOrigin(Deno.env.get("APP_URL"));
...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  const origin = allowedAppOrigin(requestOrigin, configuredOrigin);
  ...
  if (
    !supabaseUrl || !anonKey || !serviceKey || !hashPepper || !configuredOrigin
  ) {
